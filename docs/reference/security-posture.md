...
GitHub secret scanning and push protection are free for public repositories. On private and internal repositories they are paid GitHub Secret Protection features (currently billed per active committer). The defaults therefore keep the free public features on while pinning paid and private/internal features off. Dependabot vulnerability alerts default off as a client preference and are enforced off per repository via `vulnerability_alerts`. Dependabot **security updates are not managed per repository at all** (removed 2026-07-19): this org uses Renovate exclusively for dependency updates, so the only Dependabot enforcement is the organization-level `dependabot_alerts` / `dependabot_security_updates` defaults for NEW repositories, both pinned false.
