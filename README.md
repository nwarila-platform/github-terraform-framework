...
- **`provider_gaps` is declaration-only.** It is a top-level repository key, never a child of `actions`. Terraform projects these values into the planned `provider_gap_desired_state` output, but no resource consumes them. The detector compares that projection with GET-only API reads and reports mismatches through the existing sanitized drift issue; it does not apply provider-gap settings.
