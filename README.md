...
- **`require_code_owner_review` requires an effective CODEOWNERS source.** Either set `codeowners: |` per-repo in YAML (required for org mode), set `var.repo_default_codeowners`, or run in personal-account mode (`github_is_organization = false`) so the framework synthesizes `* @<github_owner>` automatically. Direct Terraform writes to `.github/CODEOWNERS` are disabled by default via `manage_codeowners_files = false` because established protected repos often require PRs and signed commits. Enable it only during controlled bootstrap or where the automation identity can write directly.
